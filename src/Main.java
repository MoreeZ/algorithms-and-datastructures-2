...
public class Main {
    public static void main(String[] args) {
        ...
        // load stops globally as they are used in every function so we only need to do it once.
        ...
        while (!quit) {
            ...
            System.out.println("(3) Trip search using arrival time");
    ...
    static void runShortestPath(Scanner input, HashMap<Integer, Stop> stops) {
        ...
        while (!exit) {
            ...
            if (searchInput.equalsIgnoreCase("exit")) {
                ...
                System.out.println("[step] stop_name (stop_id) - distance: distance from last stop");
                // Implement functionality of tracing back the shortest path.
                LinkedList<Integer> path = new LinkedList<>();
                // add the first index to path
                path.addFirst(IDtoIndexMap.get(inputTo));
                // get the previous vertex from the array generated by dijkstra algorithm
                int prevVer = dijkstra.previousArr[IDtoIndexMap.get(inputTo)];
                // trace back the previous vertex array until the starting point is reached
                while (prevVer != -1) {
                    path.addFirst(prevVer);
                    prevVer = dijkstra.previousArr[prevVer];
                }
                // print out each stop information relevant to the path
                ...
                // Allow user to read the data by stopping the loop until Enter is pressed
    ...
    private static int validateBusIdInput(String searchInput, HashMap<Integer, Stop> stops) {
        try {
            ...
            if (stops.containsKey(parsed)) {
                ...
                System.out.println("Bus stop "+ searchInput+" does not exist! Please try again.");
                return -1;
            }
        } catch (Exception e) {
    ...
    static void runStopSearch(Scanner input, HashMap<Integer, Stop> stops) {
        ...
        while (!exit) {
            ...
            if (searchInput.equalsIgnoreCase("exit")) {
                ...
                if (stopData != null) {
                    // Print out all stop information
    ...
    static void runSearchTrips(Scanner input, HashMap<Integer, Stop> stops) {
        ...
        while (!exit) {
            ...
            if (arrivalTimeSearchIn.equalsIgnoreCase("exit")) {
                ...
                if (m.matches()) {
                    System.out.println("Loading full trips containing the arrival time \"" + arrivalTimeSearchIn + "\"...");
